...
public class DrawableUniverse implements ActionListener {
    ...
    protected boolean _drawStatistics = true, _drawRings = false,
            _drawBoundRadius = true, _drawOrigin = true, _drawParticles = true,
            _drawScale = true;
    ...
    public DrawableUniverse(JPanel panel) {
        universe = new Universe(new Vector<IParticle>());
        universe.temp = this;
        this.panel = panel;
    }
     ...
     */
    public void step() {
        // FIXME Separate GUI draws from simulation
        // universe.step();

        if (panel != null && !panel.isPaintingTile()) {
            panel.repaint();
        }
    }
