...
public class DrawableParticle {
    ...
    public void draw(Graphics2D g) {        
        Vector1x3 position = particle.getPosition();
        ...
        if (_drawVelocity) {
	        ...
	        g.drawLine(x,y,x+(int)(particle.getVelocity().x),y+(int)(particle.getVelocity().y));
        }
 
        if (_drawAccel) {
        	g.setColor(Color.MAGENTA);
        	g.drawLine(x,y,x+(int)(particle.getAcceleration().x * 20),y+(int)(particle.getAcceleration().y * 20));
        }
        
        if (_drawRadius) {
        	g.setColor(Color.DARK_GRAY);
        	//int radius = (int) (particle.getRadius() * scale);
        	int radius = (int) ((Math.abs(particle.getPosition().z + 20000) / 40000) * particle.getRadius());
